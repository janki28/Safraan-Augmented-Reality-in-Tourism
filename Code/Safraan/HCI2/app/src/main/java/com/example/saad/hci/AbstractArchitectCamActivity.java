...
public abstract class AbstractArchitectCamActivity extends Activity implements ArchitectViewHolderInterface {
	...
	public void onCreate( final Bundle savedInstanceState ) {
		...
		this.architectView = this.findViewById( this.getArchitectViewId()  );
			...
			final Thread t = new Thread(new Runnable() {
				...
				public void run() {
					...
					if (lastKnownLocaton!=null && !isFinishing()) {
						// you may replace this dummy implementation and instead load POI information e.g. from your database
						poiData = getPoiInformation(lastKnownLocaton, 20);
						if (poiData != null) {
							callJavaScript(new String[] { poiData.toString() });
						}
					}
	 ...
	 */
	private void callJavaScript(final String[] arguments) {
		...
		if (this.architectView!=null) {
			final String js = ( "World.loadPoisFromJsonData" + "( " + argumentsString.toString() + " );" );
	...
	public static JSONArray getPoiInformation(final Location userLocation, final int numberOfPlaces) {
		AbstractArchitectCamActivity.numberOfPlaces = numberOfPlaces;
	...
	private static double[] getRandomLatLonNearby(final double lat, final double lon) {
		AbstractArchitectCamActivity.lat = lat;
		AbstractArchitectCamActivity.lon = lon;
