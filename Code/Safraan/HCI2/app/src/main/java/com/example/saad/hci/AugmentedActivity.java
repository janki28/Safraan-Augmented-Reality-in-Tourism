...
public class AugmentedActivity extends ListActivity{
	...
	protected void onCreate( Bundle savedInstanceState ) {
		...
		try {
            ...
            this.startActivity(intent);
                 ...
                 * listed in manifest
                 */
            Toast.makeText(this, className + "\not defined/accessible",
                    Toast.LENGTH_SHORT).show();
        }
		if(isVideoDrawablesSupported())
		{
			Toast.makeText(this, R.string.videosrawables_fallback, Toast.LENGTH_LONG).show();
		}this.setTitle( this.getActivityTitle() );
		this.getActivitiesGeo();
		this.getActivitiesIr();
		this.getArchitectWorldUrls();
		this.getListActivities();
		this.getListFrom("");
		this.getContentViewId();
	}

	private void getListFrom(String fname) {
     ...
     * recommended to check before launching ARchitect Worlds with videodrawables
	 * @return true if AR.VideoDrawables are supported,
     * false if fallback rendering would apply (= show video fullscreen)
	 */
	public static  boolean isVideoDrawablesSupported() {
		...
		return Objects.requireNonNull(getIntent().getExtras()).getString(
				EXTRAS_KEY_ACTIVITY_TITLE_STRING);
	}

	protected String[] getListActivities() {
		return Objects.requireNonNull(getIntent().getExtras()).getStringArray(
	...
	protected boolean[] getActivitiesIr() {
		return Objects.requireNonNull(getIntent().getExtras()).getBooleanArray(
    ...
    protected void onResume() {
        super.onResume();
		Toast.makeText(this, "onresume executed in augmented activity",
				Toast.LENGTH_SHORT).show();
        finish();
    }
}
