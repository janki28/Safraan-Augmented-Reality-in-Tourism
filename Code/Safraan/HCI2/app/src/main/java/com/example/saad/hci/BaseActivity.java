...
import android.content.pm.PackageInfo;
        ...
        import androidx.core.content.ContextCompat;import com.mapbox.vision.mobile.core.utils.SystemInfoUtils;
        import com.mapbox.vision.mobile.core.utils.snapdragon.SupportedSnapdragonBoards;
        import com.mapbox.vision.utils.VisionLogger;public abstract class BaseActivity extends AppCompatActivity {    private static final String PERMISSION_FOREGROUND_SERVICE = "android.permission.FOREGROUND_SERVICE";
    private static final int PERMISSIONS_REQUEST_CODE = 123;    protected abstract void onPermissionsGranted();    protected abstract void initViews();    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);        String board = SystemInfoUtils.INSTANCE.getSnpeSupportedBoard();
        if (!SupportedSnapdragonBoards.isBoardSupported(board)) {
            ...
            finish();
            return;
        }        initViews();        setTitle(getString(R.string.app_name) + " " + this.getClass().getSimpleName());        if (!allPermissionsGranted() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
