...
public class SamplePluginActivity extends AbstractArchitectCamActivity {
	...
	public String getARchitectWorldPath() {
		return Objects.requireNonNull(getIntent().getExtras()).getString(
				AugmentedActivity.EXTRAS_KEY_ACTIVITY_ARCHITECT_WORLD_URL);
	}

	@Override
	public String getActivityTitle() {
		return (getIntent().getExtras() != null && getIntent().getExtras().get(
				AugmentedActivity.EXTRAS_KEY_ACTIVITY_TITLE_STRING) != null) ? getIntent()
				.getExtras().getString(AugmentedActivity.EXTRAS_KEY_ACTIVITY_TITLE_STRING)
				: "Test-World";
	}
	...
	public SensorAccuracyChangeListener getSensorAccuracyListener() {
		return new SensorAccuracyChangeListener() {
			...
			public void onCompassAccuracyChanged( int accuracy ) {
				...
				if (accuracy < SensorManager.SENSOR_STATUS_ACCURACY_MEDIUM && !SamplePluginActivity.this.isFinishing() && System.currentTimeMillis() - SamplePluginActivity.this.lastCalibrationToastShownTimeMillis > 5 * 1000) {
	...
	protected boolean hasGeo() {
		return Objects.requireNonNull(getIntent().getExtras()).getBoolean(
				AugmentedActivity.EXTRAS_KEY_ACTIVITY_GEO);
	}
	...
	protected boolean hasIR() {
		return Objects.requireNonNull(getIntent().getExtras()).getBoolean(
				AugmentedActivity.EXTRAS_KEY_ACTIVITY_IR);
	}
